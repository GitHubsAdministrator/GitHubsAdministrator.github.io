...
<div class="container" style="margin-top: 23px;">
	<div class="left">
		...
		<div class="wrap-box" style="position:relative">
			...
			<?php if (isset($_GET['HY_URL'][2])): ?><?php if ($_GET['HY_URL'][2] == 'new'): ?>默认排序<?php else: ?><?php echo $_LANG['最新回复'];?><?php endif ?><?php else: ?>默认排序<?php endif ?> <i class="iconfont icon-sort"></i>
			...
			<button onclick="location.href='<?php HYBBS_URL('forum',$fid,'btime'); ?>'" class="btn select-option"><?php echo $_LANG['最新回复'];?></button>
			</div>
			<?php if (IS_LOGIN): ?>
			<div class="pull-right">
				<a href="<?php HYBBS_URL('post'); ?>?fid=<?php echo $fid;?>" class="btn btn-link"><i style="font-size: 14px;margin-right: 5px;" class="iconfont icon-edit"></i>发表帖子</a>
			</div>
			<?php endif ?>
			</div>
    ...
    <div class="middle text">
		...
		<div class="wrap-box">
			...
			<a href="<?php if ($pageid==1): ?>javascript:void(0);<?php else: ?><?php HYBBS_URL('forum',$fid,[$type=>$pageid-1]); ?><?php endif ?>"  class="btn btn-primary <?php if ($pageid==1): ?>disabled<?php endif ?>" >上一页</a>
			<a  href="<?php if ($pageid==$page_count): ?>javascript:void(0);<?php else: ?><?php HYBBS_URL('forum',$fid,[$type=>$pageid+1]); ?><?php endif ?>" class="btn btn-primary pull-right <?php if ($pageid==$page_count): ?>disabled<?php endif ?>" >下一页</a>
